<template>
  <Row
    :ratio="ratio"
    class="w-full px-2 group min-h-row-mid items-stretch py-1.5"
    :class="readOnly ? '' : 'hover:bg-gray-25 dark:hover:bg-gray-900'"
  >
    <!-- Index or Remove button -->
    <div
      class="flex min-w-0 items-center self-stretch ps-2 text-gray-600 dark:text-gray-400"
    ...
    <div
      ...
      :key="df.fieldname"
      class="flex min-h-0 min-w-0 self-stretch px-1"
      :class="isNumeric(df) ? 'justify-end' : 'justify-start'"
    >
      <FormControl
    ...
    <div
      v-if="canEditRow"
      class="flex min-w-0 items-center justify-center self-stretch"
    >
      <Button
        :icon="true"
        :padding="false"
