    ...
    <div :class="border ? 'border dark:border-gray-800 rounded-md' : ''">
      ...
      >
        ...
        <div
          ...
          :key="df.fieldname"
          class="flex min-h-row-mid px-2 py-1.5"
          :class="[
            df.sub_label
...
export default {
  ...
  computed: {
    ...
    ratio() {
      const ratio = [0.32].concat(
        this.tableFields.map((df) => this.fieldColumnWeight(df)),
      );

      if (this.canEditRow) {
        return ratio.concat(0.34);
      }
