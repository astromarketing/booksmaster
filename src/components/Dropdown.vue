<template>
  ...
  >
    ...
    <template #content>
      <div
        data-books-dropdown-panel
        :class="[
          'bg-white dark:bg-gray-850 dark:text-white rounded w-full overflow-hidden',
          wide ? 'min-w-72' : 'min-w-40',
        ]"
      >
        <div
          class="
        ...
        >
          <slot name="footer" :toggle-dropdown="toggleDropdown"></slot>
        </div>
      </div>
    </template>
  </Popover>
  ...
  methods: {
    onOutsideClick(e: Event) {
      ...
      for (const node of path) {
        ...
        if (
          node.hasAttribute('data-books-dropdown-panel') ||
          node.classList.contains('popover-container')
        ) {
          return;
        }
      }
