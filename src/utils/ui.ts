...
import { exportSingleDocToXlsxBase64 } from './export';
...
function getSingleInvoiceExcelExportAction(doc: Doc): Action {
  return {
    label: t`Export to Excel`,
    group: t`Export`,
    condition: (d: Doc) => d.inserted && !!d.name,
    async action() {
      try {
        const base64 = await exportSingleDocToXlsxBase64(
          doc.schemaName,
          doc.name!,
          fyo
        );
        const label =
          fyo.schemaMap[doc.schemaName]?.label ?? doc.schemaName;
        const safeName = `${label}-${doc.name}`
          .replace(/[/\\?%*:|"<>]/g, '-')
          .replace(/\s+/g, '-');
        const { canceled, filePath } = await getSavePath(safeName, 'xlsx');
        if (canceled || !filePath) {
          return;
        }
        await ipc.saveData(base64, filePath, 'base64');
        fyo.telemetry.log(Verb.Exported, doc.schemaName, {
          extension: 'xlsx',
          singleDoc: true,
        });
        ...
        showToast({
          message: `${fyo.t`Export failed`}: ${getErrorMessage(e as Error)}`,
          type: 'error',
        });
      }
    },
...
export function getActionsForDoc(doc?: Doc): Action[] {
  ...
  if (
    doc.schemaName === ModelNameEnum.SalesInvoice ||
    doc.schemaName === ModelNameEnum.PurchaseInvoice
  ) {
    actions.unshift(getSingleInvoiceExcelExportAction(doc));
  }
