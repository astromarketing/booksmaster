...
export abstract class Invoice extends Transactional {
  ...
  /** Sum of line amounts; overridden on Sales Invoice to include freight. */
  getItemsNetTotal(): Money {
    return this.getSum('items', 'amount', false) as Money;
  }
  ...
  formulas: FormulaMap = {
    ...
    netTotal: {
      formula: () => this.getItemsNetTotal(),
      dependsOn: ['items', 'applyFreight', 'freightAmount'],
    },
    taxes: {
      formula: async () => await this.getTaxSummary(),
      dependsOn: [
        'items',
        'discountAfterTax',
        'applyFreight',
