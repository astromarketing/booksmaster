...
export abstract class InvoiceItem extends Doc {
  static lists: ListsMap = {
    description(doc?: Doc) {
      return InvoiceItem.getSalesLineDescriptionOptions(doc);
    },
  };

  static getSalesLineDescriptionOptions(doc?: Doc): string[] {
    if (!doc?.fyo) {
      return [];
    }
    const line = doc as InvoiceItem;
    if (!line.isSales) {
      return [];
    }
    const raw = doc.fyo.singles.Defaults?.salesInvoiceItemDescriptionOptions;
