 ...
 */
function getAvailablePort(startFrom, hostname, maxTries = 64) {
  return new Promise((resolve, reject) => {
    (function tryPort(p) {
      if (p >= startFrom + maxTries) {
        reject(
          new Error(
            `No free TCP port between ${startFrom} and ${startFrom + maxTries - 1} on ${hostname}`
          )
        );
        return;
      }
...
process.env['VITE_HOST'] = process.env['VITE_HOST'] || '127.0.0.1';

const preferredPort = Number(process.env['VITE_PORT']) || 6969;
const vitePort = await getAvailablePort(preferredPort, process.env['VITE_HOST']);
if (vitePort !== preferredPort) {
  console.log(
    `Note: port ${preferredPort} is already in use; dev server will use ${vitePort}`
  );
}
