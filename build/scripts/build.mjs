...
copyPackageJson();
ensurePackagingPlaceholderFiles(root);
installProductionDependenciesInAppBuild(buildDirPath);
 ...
 */
/**
 * electron-builder copies `log_creds.txt` into the package; CI creates it from secrets.
 * Local builds use an empty placeholder so packaging does not warn or skip the resource.
 */
function ensurePackagingPlaceholderFiles(root) {
  const credsPath = path.join(root, 'log_creds.txt');
...
/**
 * Install production dependencies into `dist_electron/build` so native modules (e.g.
 * better-sqlite3) live under the app directory. electron-builder then rebuilds them for
 * the **target** OS (e.g. win32) instead of only seeing the repo-root Linux `node_modules`.
 */
function installProductionDependenciesInAppBuild(appBuildDir) {
